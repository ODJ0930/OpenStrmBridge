...
| 版本              | 主要更新                                                                                                                                                          |
| ----------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `v0.1.9` 当前构建 | 统一 AI 重命名与 STRM 任务完成状态；无失败时跳过项按正常完成处理；历史跳过任务状态自动修正；运行结果阶段文案更清晰                                                |
| `v0.1.8`          | AI 重命名递归逻辑媒体分组；深层剧集容器与季目录识别；分类层级保留；托管任务、立即任务及 STRM 前重命名统一增量基线                                                 |
...
| Emby 标准结构    | 电视剧整理为 `剧名 (年份)/Season 01/剧名 - S01E01.ext`；电影及多版本整理为同一电影目录和唯一版本标签；正确区分电影合集和电视剧季集 |
...
- AI 设置页可自定义单次请求的视频上限（1–1000，默认 100）；超过上限会自动顺序分批，增量跳过的媒体组不会进入分批。
...
任务先递归拉取目标目录并建立清单、完成增量过滤，再把本次新增或变化的逻辑媒体组顺序提交给 LLM。单次请求最多包含设置页配置的上限（默认 `100` 个视频），超过上限时自动拆成多批；同一个大型媒体组也会按视频拆分，模型结果返回后由后端合并。后端再生成实际名称和目标路径，并按顺序逐项修改，既避免模型上下文过大，也避免大量并发改名触发云盘接口限制。

遇到瞬时错误、结构化输出回退或请求重试时，单个批次可能产生额外模型请求；正常识别流程不会按每个文件单独调用 LLM。任务日志会显示总批次数和当前 `批次/总批次`。

### 增量识别

- 第一次运行建立全量基线。
- 后续运行根据文件路径、名称、大小和修改时间计算目录指纹。
- 只把新增或变化的逻辑媒体组提交给 AI；未变化媒体组在分批前直接计入“增量跳过”，不会占用请求 Token。
...
同一电影的多个文件会保留可区分的版本标签；AI 未提供标签时，后端会从原文件名确定性提取分辨率、DV/HDR、片源、编码、音频和发布组。无法提取时使用稳定的 `Version 1/2` 后备名称，不再因目标名称相同而整组跳过。开启“重建 Emby 标准文件夹结构”后，多版本会移动到同一个电影目录。

程序保留原始扩展名、多集编号、`v2` 和分段信息；单版本内容会移除网站广告、发布组、分辨率和编码噪声。字幕不会提交给 AI：后端会在视频名称确定后按原始同名基名、季集编号或单视频目录关系自动关联，并直接改成视频的最终基名，同时保留 `.zh-CN`、`.forced`、`.sdh` 等字幕标签。NFO、海报等其他附属文件仍只在能够可靠关联视频时跟随处理。
...
同目录的 `srt`、`ass`、`ssa`、`sub`、`vtt` 外挂字幕会从本地、OpenList / Alist 或 WebDAV 同步到 STRM 输出目录，目标文件始终使用对应 `.strm` 的基名，并保留语言、强制字幕和听障字幕标签。增量任务会跳过内容未变化的字幕；源字幕删除、改名或重新关联后，完整扫描会通过 STRM 索引安全清理失效字幕。

失效清理遵循以下边界：

1. 仅完整扫描成功后比较本次结果和持久化 STRM 索引。
2. 云盘源文件或字幕改名、删除后，旧 STRM 和旧字幕会在成功扫描结束时清理。
3. 同一 STRM 或字幕被其他任务共享时只解除当前任务关联，不误删仍被引用的文件。
4. 索引路径越过任务输出目录、目标不是受管理的 STRM/字幕，或扫描失败/被限制时拒绝清理。
