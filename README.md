...
| AI 重命名 | 探测 OpenAI 兼容模型并测试延迟/Token 速度，先汇总整库目录清单进行一次性 AI 识别，再逐项安全修改；支持自定义 Chat Completions 参数、提示词、中英命名顺序、TMDB 校验和季目录合并 |
| 任务管理  | 按存储路径生成 STRM，可选在扫描前先执行增量 AI 重命名（只提交新增或变化目录）；完整扫描成功后自动清理失效 STRM，支持增量生成、目录时间检查、OpenList 缓存预刷新                |
...
| API 接口  | 提供外部 API 开关、密钥重置、鉴权说明和完整接口清单                                                                                                                            |
...
| Proxy    | vendored go-emby2openlist source，开发时源码启动，发行包和容器使用预编译二进制 |
...
Docker Hub 镜像：[diead/openstrmbridge](https://hub.docker.com/r/diead/openstrmbridge)

当前版本镜像同时支持 `linux/amd64` 和 `linux/arm64`：

```bash
docker pull diead/openstrmbridge:v0.1.7
```
...
```text
./docker-data  -> /app/data  # 设置、存储、任务和增量状态
./docker-strm  -> /app/strm  # 生成的 STRM 文件
```
...
| 场景                      | Docker 配置                                                                                        |
| ------------------------- | -------------------------------------------------------------------------------------------------- |
| 更换管理端口              | 将 `-p 5174:5174` 改为 `-p 新端口:5174`                                                            |
| 使用本地媒体目录          | 增加 `-v /宿主机媒体目录:/media`，在存储管理中选择容器路径 `/media`                                |
| 与 Emby 共享 STRM         | 把同一个宿主机 STRM 目录挂载到 Emby 的 `/media/strm`，系统中的 Emby 媒体挂载路径保持 `/media/strm` |
| 安装神医助手到宿主机 Emby | 挂载 Emby 插件目录并设置 `OPENSTRMBRIDGE_EMBY_PLUGIN_DIR` 为对应容器路径                           |
| 访问宿主机服务            | Linux Compose 已配置 `host.docker.internal:host-gateway`，接口地址可使用 `host.docker.internal`    |
...
`docker-data` 和 `docker-strm` 使用宿主机目录持久化，重新拉取镜像不会删除现有配置和 STRM 文件。
...
3. 如需整理已有媒体，先在「AI 自动重命名」直接配置兼容接口、可编辑提示词和命名规则，再到「AI 重命名任务」选择目录、保存任务并统一运行。
4. 在「系统设置 / 302代理」填写 Emby 服务地址，并确认代理入口端口。
5. 在「任务管理」选择存储路径，创建 STRM 生成任务；需要自动整理时，可开启“生成 STRM 前先执行 AI 重命名”。
6. 手动运行一次任务确认输出，之后交给后台调度器按 cron 自动执行。
...
server/
  ai-rename-core.mjs         # AI 结构化结果校验和媒体库名称渲染
...
| `OPENSTRMBRIDGE_EMBY_PLUGIN_DIR`            | 自动探测                   | Emby 插件目录；Docker 部署时可显式挂载并设置 |
